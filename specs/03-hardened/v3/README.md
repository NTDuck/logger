# Feature Specification: Log Collection and Application Error Monitoring System (Hardened Design v3)

**Feature Branch**: `v3-hardened`

**Created**: 2026-06-22

**Status**: Ready

**Input**: User description: "Create specs/03-hardened/v3/README.md... Ensure absolute compliance to specs/02-disambiguated/README.md... Rewrite every section identified by the council to eliminate ambiguity, enforce absolute boundaries, and satisfy their negative constraints."
...
* **Acceptance Scenarios**:
  ...
  2. **Given** a client requests a WebSocket connection with an expired or invalid token signature, **When** the handshake occurs, **Then** the server MUST reject the connection with a HTTP `401 Unauthorized` equivalent response.
...
* **Independent Test**: Expose the Edge API receiver, submit valid OTLP payloads, recursive payloads (depth > 5), and large payloads (>64KB). Verify that valid payloads are successfully indexed in ClickHouse, while malformed payloads are rejected or routed to the DLQ.
* **Acceptance Scenarios**:
  1. **Given** a valid OTLP JSON payload with nested key-value arrays, **When** it hits the Edge Receiver, **Then** it MUST be parsed into memory (only after passing the 1MB payload check), mechanically unrolled into dot-notation flat structures, and successfully published to `logs-raw`.
  2. **Given** a log payload containing dynamic attributes with a nesting depth of 6 or higher, **When** the Normalization Worker processes it, **Then** the message MUST be identified as a "Poison Pill" and routed to `logs-dlq` within 500ms.
...
- **Edge Receiver Fast-Fail Memory Boundary**: The Edge Receiver HTTP/gRPC ingress MUST enforce a strict `max_body_size = 1048576` (1MB) connection-level read boundary *before* initiating any JSON payload deserialization. Payloads exceeding this limit MUST result in immediate connection termination (HTTP 413) to prevent OOM attacks.
...
- **Redis Alert Deduplication Cardinality Limit**: The Alert Consumer MUST enforce a hard O(1) space cap of exactly `10,000` unique error fingerprints per 60-second window in Redis. If this maximum capacity is reached, further unique errors MUST fall back to a batch summary counter (`overflow_count`) to prevent unbounded memory allocation.
...
- **ClickHouse Write Outage**: If ClickHouse becomes temporarily unavailable, the DB Writer MUST retry batch inserts using exponential backoff up to a maximum duration, then halt consumption.
...
- **FR-001**: The Edge Receiver MUST authenticate incoming gRPC OTLP and HTTP POST log payloads using a stateless JWT Authorization header. It MUST execute zero business logic beyond authentication and payload parsing. Once the JSON payload is safely parsed (post-1MB check), it MUST mechanically unroll OTLP nested `kvlists` into flat structures and proxy the payload to `logs-raw`.
- **FR-002**: Client SDKs MUST strip PII before transmission. To mitigate unredacted client failure, `logs-raw` MUST be configured at the broker level with strict short retention (`retention.ms=86400000`, i.e., 24 hours). The Normalization Worker MUST execute statically compiled regex-based PII redaction rules (baked into the worker binary at build time to prevent configuration I/O blocks) against the parsed AST prior to publishing to `logs-normalized`. 
- **FR-003**: The Normalization Worker MUST route logs with level `ERROR` or `CRITICAL` directly to `alerts-priority-stream`.
...
- **FR-007**: The WebSocket Server MUST use the Broadcast Consumer Pattern to scaling horizontally, parsing JWT `app_grants` claims to filter messages in-memory, without querying any database or cache on a per-message basis.
...
- **FR-010**: An **Admin API Actor** MUST be implemented to receive administrative HTTP POST configuration requests. This actor writes configurations to `alert_configs` ReplacingMergeTree table in ClickHouse and publishes updates via Redis Pub/Sub to the Alert Consumer.
...
| :--- | :--- | :--- |
| **Max Depth** | 5 levels (derived from dot-notation key segments) | Message classified as Poison Pill -> DLQ |
| **Payload Size** | 64KB compressed | Message classified as Poison Pill -> DLQ |
  ...
  /v1/logs:
    post:
      ...
      responses:
        ...
        '413':
          description: Payload exceeds 1MB limit.
components:
  securitySchemes:
    bearerAuth:
      type: http
...
graph TD
    ...
    Edge -->|Produces Flattened| TopicRaw["logs-raw (Topic)"]
    
    Worker[Normalization Actor Deployment] -->|Consumes| TopicRaw
    Worker -->|Produces Normalized| TopicNorm["logs-normalized (Topic)"]
    Worker -->|Produces Alerts| TopicAlert["alerts-priority-stream (Topic)"]
...
To perfectly support dynamic flattened data without breaking analytical speeds or reverting OTLP attribute projections, ClickHouse will utilize parallel Array columns (`attribute_keys` and `attribute_values_string`) rather than forcing arbitrary serialization into a strict `Map(String, String)`.
...
When a consumer encounters a processing failure, it MUST wrap the original payload in this exact schema envelope before publishing to `logs-dlq`:
...
Both the Edge Receiver (Telemetry Ingestion) and the WebSocket server (Viewer Output) MUST enforce stateless RBAC utilizing JWT tokens validated entirely in-memory using shared public keys. No database lookups are permitted.
...
- **Maintained Surfaces**: Feature specification (`v3/README.md`).
...
- **Evidence/Rationale**:
  ...
  - Trust boundary identified: The Edge Receiver now enforces a hard authentication wall (JWT) prior to processing any JSON AST into memory, rectifying the ingestion vulnerability.
...
- **Evidence/Rationale**:
  ...
  - Blocked network I/O checks passed.
