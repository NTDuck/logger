# Feature Specification: Log Collection and Application Error Monitoring System (Hardened Design v4)

**Feature Branch**: `v4-hardened`

**Created**: 2026-06-22

**Status**: Ready

**Input**: User description: "Create specs/03-hardened/v4/README.md... Ensure absolute compliance to specs/02-disambiguated/README.md... Rewrite every section identified by the council to eliminate ambiguity, enforce absolute boundaries, and satisfy their negative constraints."
...
* **Independent Test**: Expose the Edge API receiver, submit valid OTLP payloads, recursive payloads (depth > 5), heterogenous arrays, and large payloads (>64KB). Verify that valid payloads are successfully indexed in ClickHouse, while malformed payloads are routed to the DLQ.
* **Acceptance Scenarios**:
  1. **Given** a valid OTLP JSON payload with nested key-value arrays matching the JWT `app_grants`, **When** it hits the Edge Receiver, **Then** it MUST be parsed into memory (only after passing the 1MB payload check), mechanically unrolled into dot-notation flat structures (with the edge recording the segment depth), and successfully published to `logs-raw`.
  2. **Given** a log payload containing dynamic attributes where the edge recorded a nesting depth of 6 or higher, **When** the Normalization Worker processes it, **Then** the message MUST be evaluated, identified as a "Poison Pill", and routed to `logs-dlq` within 500ms.
...
- **FR-001**: The Edge Receiver MUST authenticate incoming gRPC OTLP and HTTP POST log payloads using a stateless JWT Authorization header. It MUST verify authorization by ensuring the payload's `app_name` is present in the JWT `app_grants` array (or if the JWT contains the `*` wildcard). If unauthorized, reject with HTTP 403. Once the JSON payload is safely parsed (post-1MB check), it MUST mechanically unroll OTLP nested `kvlists` into flat structures, append the calculated key segment depth, and proxy the payload to `logs-raw`.
- **FR-002**: Client SDKs MUST strip PII before transmission. To mitigate unredacted client failure, `logs-raw` MUST be configured at the **Redpanda topic level** with strict short retention (`retention.ms=86400000`, i.e., 24 hours). The Normalization Worker MUST execute statically compiled regex-based PII redaction rules (baked into the worker binary at build time to prevent configuration I/O blocks) against the parsed AST prior to publishing to `logs-normalized`. 
...
- **FR-007**: The WebSocket Server MUST use the Broadcast Consumer Pattern to scaling horizontally, parsing JWT `app_grants` claims to filter messages in-memory, without querying any database or cache on a per-message basis. It MUST natively support the `*` wildcard claim to allow administrators to receive logs from all applications.
...
| :--- | :--- | :--- |
| **Max Depth** | 5 levels (derived from dot-notation key segments by the Edge Receiver and evaluated by the Normalization Worker) | Message classified as Poison Pill -> DLQ |
| **Payload Size** | 64KB compressed | Message classified as Poison Pill -> DLQ |
| **Homogeneous Arrays** | Arrays MUST NOT contain mixed data types (e.g., integers and strings together). | Message classified as Poison Pill -> DLQ |
...
To perfectly support dynamic flattened data without breaking analytical speeds or reverting OTLP attribute projections, ClickHouse will utilize parallel Array columns (`attribute_keys` and `attribute_values_string`). The schema explicitly relies on conditional `DELETE WHERE` statements for proper tiering without causing catastrophic silent data deletion.
...
- **Maintained Surfaces**: Feature specification (`v4/README.md`).
