...
- **The HTTP Boundary — True Token-Stream Parsing:**
  The ingestion pipeline MUST NOT deserialize the raw payload into an intermediate AST model like `WireLog` or `serde_json::Value`. The payload must be evaluated as a raw byte stream using a low-level token pull-parser (e.g., `struson` or a custom byte-scanner) to count `{` and `[` tokens directly from the byte stream. This prevents memory exhaustion and DoS attacks by aborting prematurely on deeply nested JSON BEFORE any memory allocation occurs if depth > 5.
...
- **Step 2 — Pure Logic (Streaming JSON Validator & Flattener & JWT Validator):**
  1. Implement the true token-stream JSON depth validator and flattener:
     - Signature: Takes a reference to the raw HTTP body bytes, returns Result of `DomainLog` or `EdgeError::BadRequest`.
     - Mechanism: The DAG explicitly instructs the use of a low-level token pull-parser (e.g., `struson` or a custom byte-scanner). You MUST count `{` and `[` tokens directly from the byte stream, aborting before ANY memory allocation if depth > 5.
     - You MUST eradicate `serde_json::Deserializer::from_slice(bytes).into_iter::<serde_json::Value>()` or any instruction to deserialize into `serde_json::Value` or `WireLog`.
...
- **Step 4 — The Axum Handler (The Actor Loop):**
  1. Define the handler function for POST "/v1/logs".
     ...
     - The DAG explicitly mandates the mechanical use of `tokio_util::sync::CancellationToken` within the handler for idempotent shutdown.
     - You MUST explicitly instruct the use of `tower::timeout::TimeoutLayer` directly within the DAG (Phase 3) to strictly govern the HTTP stream-read phase, rather than hiding it in Phase 4. Apply this timeout directly around the stream reading logic to sever Slowloris attacks idempotently.
  ...
  7. Cancellation-Safe Kafka Production & Telemetry:
     - Once the payload is validated, the `KafkaLogProducer.produce` phase MUST be wrapped in a guaranteed-completion future or spawned `tokio::task` so it cannot be cancelled mid-flight by client disconnects.
  ...
  9. Critical Handler Invariant:
     ...
     - Because of the guaranteed-completion future wrapping the produce phase, telemetry is mathematically accurate and logs are never silently duplicated.
...
- **Wiring Directives:**
  ...
  6. Build the Axum Router:
     ...
     - Do NOT wrap the entire Axum handler in `TimeoutLayer`. Apply timeout strictly to the HTTP socket stream-reading phase to sever Slowloris attacks without cancelling the guaranteed-completion produce phase.
  ...
  8. Serve with axum::serve(listener, app). Eradicate `tokio::sync::watch::Receiver` for graceful shutdown. Use graceful shutdown integration with a `tokio_util::sync::CancellationToken` which is latch-based and safe for nested/recursive retry loop polling without deadlocks. Ensure the `CancellationToken` is polled recursively inside all inner retry loops, not just at the top-level actor loop.
...
- **Exit Gate — Track Acceptance Criteria:**
  ...
  - The `WireLog` struct and `serde_json::Value` AST constructions are STRICTLY FORBIDDEN during ingestion. Deserialization MUST explicitly use a low-level token pull-parser (e.g., `struson` or a custom byte-scanner) counting `{` and `[` directly from the byte stream, aborting before ANY memory allocation if depth > 5. Eradicate `serde_json::Deserializer::from_slice(...).into_iter::<Value>()`.
