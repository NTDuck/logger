...
2. **Infrastructure Adapters**: Implement `LogProducer` natively via `rdkafka::producer::FutureProducer`.
3. **The Event Loop**: Implement the Axum web handler. The route MUST accept raw `Bytes` to validate length before deserialization.
   - **Telemetry**: Enforce `::tracing::debug!` upon ingestion, `::tracing::error!` upon rejection. Increment Prometheus metrics `logger_edge_requests_total` and `logger_edge_errors_total`.
