...
Feature: AI Consumer Classification
  ...
  Scenario: ClickHouse sidecar is offline
    ...
    Then it MUST implement exponential backoff/retry
```
```rust
...
3. **The Event Loop**: 
   ...
   - Execute `classify`.
   - Wrap `write_tag` in `tokio-retry` exponential backoff loop.
   - Execute `publish_patch`.
   - Commit Redpanda offset.
   - **Telemetry**: `::tracing::debug!` upon success. `::tracing::error!` upon inference/write failures. Increment `logger_ai_inference_success_total` and error equivalents.
