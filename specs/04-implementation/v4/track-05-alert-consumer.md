...
3. **The Event Loops**:
   - `Config Listener Task`: Loops on `listen_for_updates`, writing to the `RwLock`.
   - `Event Processor Task`: Fetches logs, reads `RwLock` config, runs `check_and_increment` (passing TTL), executes `notify` if threshold breached.
   - **Telemetry**: `::tracing::debug!` upon firing alert. `::tracing::error!` upon Redis/Telegram failure. Metrics: `logger_alerts_fired_total`.
