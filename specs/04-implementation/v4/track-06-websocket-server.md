...
3. **The Event Loops**:
   ...
   - **Telemetry**: Sessions MUST emit `::tracing::debug!` upon connection and `::tracing::error!` upon JWT rejection or lagging drop. Increment `logger_ws_connections_active` and `logger_ws_dropped_total`.
