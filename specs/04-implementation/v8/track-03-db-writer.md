...
- **The Gherkin Feature (Database Batch Writer):**
  ...
  - Scenario 3: ClickHouse is offline — backpressure and retry.
    ...
    - And increment logger_events_processed_total with labels stage="db_writer" and status="error" for the batch size outside the retry loop.
    - And enter an exponential backoff retry loop (base 1 second, max 60 seconds, with jitter).
    - And MUST continually poll `consumer.recv()` and `shutdown_rx` within a `tokio::select!` inside the retry loop to prevent rdkafka broker eviction and ensure select-safe cancellation.
...
- **Step 4: The Actor Loop — run_db_writer_loop.**
  - Function signature: async fn run_db_writer_loop(consumer: rdkafka StreamConsumer, writer: impl ClickHouseWriter, metrics: DbWriterMetrics, mut shutdown_rx: watch::Receiver<()>) -> Fallible unit.
  ...
  - Enter the main loop using tokio::select! over three branches:
    ...
    - Branch C — Cancellation: shutdown_rx.changed().await. Initiate graceful shutdown.
  - **Flush Subroutine:**
    ...
    - On failure — Kafka Physical Backpressure Mechanics:
      ...
      - BEFORE entering the infinite retry loop, increment logger_events_processed_total{stage="db_writer", status="error"} by the batch length. Count the message, not the retry attempt.
      - Enter an exponential backoff retry loop. Use sleep durations: 1s, 2s, 4s, 8s, 16s, 32s, capped at 60s. Add random jitter of up to 500ms per iteration.
      - INSIDE the retry loop, the sleep MUST be wrapped in a `tokio::select!` alongside `consumer.recv()` and `shutdown_rx.changed()` (or equivalent cancellation token). 
        - If `consumer.recv()` returns, buffer or discard the result to maintain the `rdkafka` heartbeat and prevent broker eviction (since partitions are paused, this just processes control messages).
        - If `shutdown_rx.changed()` completes, abort the retry and exit gracefully immediately.
        - If the sleep completes, execute `writer.write_batch(&batch).await`. Suffix with .tap_err(|e| ::tracing::error!(error = %e, attempt = %attempt, "ClickHouse retry attempt failed")). Do NOT increment metrics inside the loop.
      - On retry success:
...
- **Wiring Directives:**
  - In apps/src/main.rs, within the "--role db-writer" branch:
    ...
    - Spawn the run_db_writer_loop as a tokio task, passing the consumer, writer, metrics handle, and a graceful shutdown receiver.
  ...
  - **Graceful Shutdown:**
    ...
    - The `watch::Receiver` MUST be recursively polled using `tokio::select!` inside ALL inner retry loops (specifically the ClickHouse HTTP insert backoff) to ensure the system can exit cleanly during prolonged ClickHouse outages.
