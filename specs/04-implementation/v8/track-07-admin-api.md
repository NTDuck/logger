...
- **Step 3: Infrastructure Adapters**
  - ClickHouse Config Appender (implements ConfigWriter::append_config):
    ...
    - If any internal retry loop is added here for network resiliency, the graceful shutdown `CancellationToken` or `watch::Receiver` MUST be passed in and polled recursively inside the loop to avoid stalling shutdown.
...
- **Exit Gate (Track Acceptance Criteria):**
  ...
  - Server shutdown utilizes `.with_graceful_shutdown()` instead of `tokio::select!`.
