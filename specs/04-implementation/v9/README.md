# Implementation Roadmap: Log Collection and Application Error Monitoring System (v9 Tracks)
...
This v9 architecture implements deep structural decoupling and Communicating Sequential Processes (CSP) to resolve the async Rust anomalies found in v8. It strictly enforces token-level JSON parsing, latch-based cancellation, decoupled consumer backpressure, outbox patterns, and isolated egress tasks to eliminate data loss, deadlocks, commit tearing, and WebSocket frame corruption.
...
- **Step 4: The Actor Loop:** Implement the Axum POST handler.
  - Read request body as Bytes first. Apply timeouts strictly to this HTTP socket stream-reading phase, not the entire handler. Measure length and increment logger_ingest_bytes_total.
  - Validate JWT.
...
- **Wiring Directives:**
  ...
  - Setup Axum Router. Apply DefaultBodyLimit layer of 256KB to enforce the limit at the socket level. Do not wrap the entire Axum router in TimeoutLayer; apply timeouts solely to the request stream reading phase.
...
- **Step 4: The Event Loops:**
  - Ingestion Loop: Consumes from logs-normalized, pushing into bounded broadcast channel of capacity 1024. Increment telemetry OUTSIDE of any retry loops. Ensure tokio_util::sync::CancellationToken is polled.
  - Session Loop: Upgraded client WebSocket tasks, subscribing to broadcast, filtering messages by app_name. Implement Decoupled WebSocket Egress: Eradicate wrapping ws.send().await in tokio::time::timeout. Instead, the Session Loop pushes broadcast messages into a local bounded mpsc channel.
  - Egress Task: A dedicated task reads from the local mpsc and calls ws.send().await. If the mpsc channel fills up (indicating the client is slow/dead), the Session Loop drops the mpsc sender, cleanly severing the connection without corrupting dropped futures.
