...
- **Step 3: Infrastructure Adapters.**
  ...
  - NormalizedProducer Adapter:
    ...
    - **Idempotent Sinks:** For all downstream produce calls, the producer MUST extract the log_id (or a hash for DLQ) and use it as the Kafka Message Key. This guarantees idempotency across retries when Dual-Writing to multiple topics.
...
- **Step 4: The Actor Architecture (Decoupled Consumer).**
  - Eradicate polling `consumer.recv()` inside retry loops. Acknowledge that `librdkafka` handles heartbeats autonomously.
  - To handle backpressure safely without data loss, structurally decouple the consumer into two tasks connected by a bounded `tokio::sync::mpsc` channel.
  - Both tasks MUST accept a `tokio_util::sync::CancellationToken` for idempotent, graceful shutdown.
  - **Task A (Fetcher):**
    - Runs in a loop using `tokio::select!` between `cancellation_token.cancelled()` and `consume()`.
    - Upon receiving a message, pushes it into the bounded `mpsc` channel. If the channel is full, this task blocks naturally via TCP backpressure, leaving pre-fetched messages safely inside `librdkafka`'s internal queues.
  - **Task B (Processor):**
    - Runs in a loop using `tokio::select!` between `cancellation_token.cancelled()` and `mpsc.recv()`.
    - The loop body executes the following mechanical sequence:
      1. Poison Pill Gate: Call is_poison_pill() on the raw bytes. If true:
         a. Construct a DLQEnvelope.
         b. Enter a retry loop for produce_dlq(). If produce fails, sleep with exponential backoff.
            - **Idempotent Retry Backoff:** The retry sleep MUST be wrapped in a `tokio::select!` loop alongside `cancellation_token.cancelled()`. Do NOT poll `consumer.recv()` here. If downstream goes offline, Task B retries in place.
      ...
      2. Deserialize the raw bytes into the upstream DomainLog struct. On deserialization failure:
         a. Construct a DLQEnvelope from the raw bytes.
         b. Enter a retry loop for produce_dlq() identical to the Poison Pill Gate.
         c. Increment logger_dlq_routed_total counter by 1 OUTSIDE any retry loop.
         d. Increment logger_events_processed_total with labels stage="normalization", status="error" by 1 OUTSIDE any retry loop.
         e. Call commit_offset() and continue to next iteration.
      3. PII Redaction: Call redact_pii() on the message field. Capture the redaction count. If greater than 0, increment logger_pii_redactions_total OUTSIDE any retry loop.
      4. Build the NormalizedLog using the bon builder.
      5. Call produce_normalized(). If it fails, enter a retry loop identical to the Poison Pill Gate.
      6. Alert Duplication Gate: If level equals "ERROR" or level equals "CRITICAL":
         a. Call produce_alert(). If it fails, enter a retry loop identical to the Poison Pill Gate.
      7. **Telemetry Isolation:** Increment logger_events_processed_total with labels stage="normalization", status="success" by 1 EXACTLY ONCE at the end of successful processing. Count the message, not the retry attempts.
      8. Call commit_offset() on the LogConsumer. On error, log via tap_err.
...
- **Wiring Directives:**
  - In the modular monolith entrypoint (apps/src/main.rs), when the CLI role flag matches "normalization":
    ...
    5. Create a bounded `tokio::sync::mpsc::channel` (e.g., capacity 100).
    6. Spawn Task A (Fetcher), passing the consumer adapter, the mpsc sender, and the global CancellationToken.
    7. Spawn Task B (Processor), passing the producer adapter, the mpsc receiver, the three metric handles, and the global CancellationToken. Note: The Processor also needs access to the consumer adapter (or a dedicated offset committer) to call `commit_offset()`.
    8. The spawned tasks MUST be held in JoinHandles and selected against the graceful shutdown CancellationToken. On shutdown signal, the tasks MUST be allowed to drain their current in-flight messages before exiting.
...
- **Exit Gate — Track Acceptance Criteria:**
  ...
  - Zero occurrences of tokio::sync::watch::Receiver for graceful shutdown; tokio_util::sync::CancellationToken MUST be used exclusively.
  ...
  - **The Consumer Architecture MUST be decoupled into Fetcher and Processor tasks to utilize TCP backpressure via bounded mpsc channels. consumer.recv() MUST NEVER be polled inside a retry loop.**
  ...
  - The consumer event loops and all async I/O methods carry #[::tracing::instrument(skip_all)].
