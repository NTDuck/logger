...
- **Step 3: Infrastructure Adapters**
  ...
  - The session_loop function (also in src/ws/handler.rs):
    ...
    - Create a local bounded `tokio::sync::mpsc` channel for egress messages (e.g., capacity 256).
    - Spawn a dedicated Egress Task that reads from the `mpsc` receiver and calls `sink.send().await`. If `sink.send` fails, the task terminates cleanly.
    - Enter a `tokio::select!` loop with three branches:
      - Branch 1 — Cancellation: `cancel_token.cancelled()`. On trigger, break the loop.
      - Branch 2 — Broadcast Receive: Await the next message from the broadcast Receiver.
        - On Ok(msg): Call should_deliver with the client config and the message. If true, serialize the payload and push it into the local `mpsc` sender using `.try_send()`. If `try_send` returns a `Full` error (indicating the client is slow/dead and the egress channel is full), emit `::tracing::error!("Egress channel full, client lagging")`, drop the `mpsc` sender, and break the loop to cleanly sever the connection without corrupting futures. On successful send, emit ::tracing::debug!(app_name = %msg.app_name, "Message queued for egress").
        - On Err(Lagged(n)): Emit ::tracing::error!(skipped = n, "Client lagging behind broadcast channel"). Break the loop.
        - On Err(Closed): The broadcast channel has been dropped. Break the loop cleanly.
      - Branch 3 — Client Message / Close Detection: Await the next incoming WebSocket message from the client `stream`. If the client sends a Close frame or the stream returns None (connection dropped), break the loop.
