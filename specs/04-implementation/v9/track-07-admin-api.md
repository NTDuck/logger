...
- **Physical Constraints:**
  ...
  - The Redis Pub/Sub channel is fire-and-forget. If no Alert Consumer is subscribed at publish time, the notification is lost. This is structurally safe because Alert Consumers MUST execute a synchronous fetch of the latest configuration from the ClickHouse database upon startup BEFORE subscribing to the Redis Pub/Sub channel to prevent Split-Brain scenarios. Hardcoded defaults are forbidden.
...
- **Telemetry Contract (Closed-World):**
  ...
  - **CRITICAL TELEMETRY RULE:** Telemetry counters MUST be incremented strictly OUTSIDE of any retry loops. Count the message (the HTTP request), not the individual I/O retry attempts.
...
- **Step 3: Infrastructure Adapters**
  - ClickHouse Config Appender (implements ConfigWriter::append_config):
    ...
    - If any internal retry loop is added here for network resiliency, the graceful shutdown `tokio_util::sync::CancellationToken` MUST be passed in and polled recursively inside the loop to avoid stalling shutdown (Idempotent Cancellation). `tokio::sync::watch::Receiver` is strictly forbidden to prevent deadlocks.
...
- **Step 4: The Actor Loop (Axum POST Handler)**
  ...
  - Execution sequence:
    ...
    6. On overall success, increment logger_events_processed_total with labels stage="admin", status="success". (Metric is incremented exactly once per request).
...
- **Wiring Directives (in apps/src/main.rs or the role dispatch block):**
  - When the CLI role flag equals "admin-api":
    ...
    8. **Cancellation Integrity:** The spawned task MUST participate in the graceful shutdown mechanism. Top-level `tokio::select!` cancellation wrapping the server is strictly forbidden, as it violently severs active connections. The Axum server MUST be configured with `.with_graceful_shutdown()` that recursively polls the `tokio_util::sync::CancellationToken`, allowing in-flight requests to complete safely. `tokio::sync::watch::Receiver` is strictly forbidden to prevent deadlocks.
...
- **Exit Gate (Track Acceptance Criteria):**
  ...
  - Server shutdown utilizes `.with_graceful_shutdown()` using `tokio_util::sync::CancellationToken` instead of `tokio::select!` or `watch::Receiver`.
